...
This module owns transient edits made to the model-copy messages before an LLM
request. It must not mutate persisted session history.
"""
            ...
            for i, message in enumerate(kept):
                ...
                for idx in range(len(non_system) - 1, -1, -1):
                    if non_system[idx].get("role") == "user":
                        kept = non_system[idx:]
                        break
                # If no user exists, provider alternation repair can still
        ...
        if not kept:
            for idx in range(len(non_system) - 1, -1, -1):
                if non_system[idx].get("role") == "user":
                    ...
                    break
            if not kept:
                kept = non_system[-1:] if non_system else []
            start = find_legal_message_start(kept)
            if start:
                kept = kept[start:]
